...
const MetaApiDialog = ({ open, onOpenChange, agentId, agentName }: MetaApiDialogProps) => {
  ...
  const handleSave = async () => {
    ...
    setLoading(true);
    try {
      // TODO: Implement META API connection
      toast({
        title: "Em breve!",
        description: "A conexão via API Oficial da META estará disponível em breve.",
      });
      onOpenChange(false);
    } catch (error) {
      ...
      toast({
        title: "Erro",
        description: "Erro ao conectar com a API da META.",
        variant: "destructive",
      });
    } finally {
      setLoading(false);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg">
        ...
        <DialogFooter>
          ...
          <Button onClick={handleSave} disabled={loading} className="bg-[#25D366] hover:bg-[#20bd5a]">
